...
class BaseValidator {
    ...
    protected function validateRequired( string $field, $value ): ?string {
        if ( $value === null || $value === '' || $value === [] ) {
            return sprintf( __( '%s is required.', 'nozule' ), $field );
        }
        return null;
    }
    ...
    protected function validateEmail( string $field, $value ): ?string {
        if ( $value && ! is_email( $value ) ) {
            return sprintf( __( '%s must be a valid email address.', 'nozule' ), $field );
        }
        return null;
    }
    ...
    protected function validatePhone( string $field, $value ): ?string {
        if ( $value && ! preg_match( '/^[\+]?[0-9\s\-\(\)]{7,20}$/', $value ) ) {
            return sprintf( __( '%s must be a valid phone number.', 'nozule' ), $field );
        }
        return null;
    }
    ...
    protected function validateDate( string $field, $value ): ?string {
        if ( $value && ! strtotime( $value ) ) {
            return sprintf( __( '%s must be a valid date.', 'nozule' ), $field );
        }
        return null;
    }
    ...
    protected function validateFutureDate( string $field, $value ): ?string {
        if ( $value && strtotime( $value ) < strtotime( 'today' ) ) {
            return sprintf( __( '%s must be a future date.', 'nozule' ), $field );
        }
        return null;
    }
    ...
    protected function validateMin( string $field, $value, $param ): ?string {
        if ( is_numeric( $value ) && $value < $param ) {
            return sprintf( __( '%s must be at least %s.', 'nozule' ), $field, $param );
        }
        if ( is_string( $value ) && strlen( $value ) < $param ) {
            return sprintf( __( '%s must be at least %s characters.', 'nozule' ), $field, $param );
        }
        return null;
    }
    ...
    protected function validateMax( string $field, $value, $param ): ?string {
        if ( is_numeric( $value ) && $value > $param ) {
            return sprintf( __( '%s must not exceed %s.', 'nozule' ), $field, $param );
        }
        if ( is_string( $value ) && strlen( $value ) > $param ) {
            return sprintf( __( '%s must not exceed %s characters.', 'nozule' ), $field, $param );
        }
        return null;
    }
    ...
    protected function validateIn( string $field, $value, $param ): ?string {
        ...
        if ( $value && ! in_array( $value, $allowed, true ) ) {
            return sprintf( __( '%s must be one of: %s.', 'nozule' ), $field, implode( ', ', $allowed ) );
        }
        return null;
    }
    ...
    protected function validateNumeric( string $field, $value ): ?string {
        if ( $value !== null && $value !== '' && ! is_numeric( $value ) ) {
            return sprintf( __( '%s must be a number.', 'nozule' ), $field );
        }
        return null;
    }
    ...
    protected function validateInteger( string $field, $value ): ?string {
        if ( $value !== null && $value !== '' && filter_var( $value, FILTER_VALIDATE_INT ) === false ) {
            return sprintf( __( '%s must be an integer.', 'nozule' ), $field );
        }
        return null;
    }
    ...
    protected function validateUrl( string $field, $value ): ?string {
        if ( $value && ! filter_var( $value, FILTER_VALIDATE_URL ) ) {
            return sprintf( __( '%s must be a valid URL.', 'nozule' ), $field );
        }
        return null;
    }
    ...
    protected function validateSlug( string $field, $value ): ?string {
        if ( $value && ! preg_match( '/^[a-z0-9]+(?:[-_][a-z0-9]+)*$/', $value ) ) {
            return sprintf( __( '%s must be a valid slug (lowercase letters, numbers, hyphens, underscores).', 'nozule' ), $field );
        }
        return null;
    }
}
